...
export type Tone = 'neutral' | 'green' | 'red' | 'amber' | 'blue' | 'purple' | 'gray';
...
export function Button({ children, onClick, disabled, variant = 'primary', type = 'button' }: { children: ReactNode; onClick?: () => void; disabled?: boolean; variant?: 'primary' | 'ghost' | 'danger'; type?: 'button' | 'submit' }) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled} style={{ ...styles[variant], padding: '7px 12px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.55 : 1 }}>
      {children}
    </button>
  );
}
...
/** Layer → badge color, so provenance is scannable. */
export function sourceTone(source: string): Tone {
  if (source === 'defaults') return 'gray';
  if (source.startsWith('env:')) return 'blue';
  if (source === 'env-vars') return 'purple';
  if (source === 'runtime') return 'green';
  return 'neutral';
}
