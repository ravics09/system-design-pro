...
import type { NotificationStatus, SubmissionStatus } from '../types';
...
export const statusTone = (s: SubmissionStatus): Tone => (s === 'accepted' ? 'green' : s === 'flagged' ? 'amber' : 'red');
export const notifTone = (s: NotificationStatus): Tone =>
  s === 'sent' ? 'green' : s === 'queued' ? 'blue' : s === 'failed' ? 'red' : 'gray';

export function Button({ children, onClick, disabled, variant = 'primary', type = 'button' }: { children: ReactNode; onClick?: () => void; disabled?: boolean; variant?: 'primary' | 'ghost' | 'danger'; type?: 'button' | 'submit' }) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled} style={{ ...styles[variant], padding: '8px 14px', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.55 : 1 }}>
      {children}
    </button>
  );
}
